...
/** Högsta list-/jämförpris som ligger över nuvarande pris (fler Awin-kolumner än bara old+rrp) */
function listPriceAboveCurrent(
...
/** Plocka ut marknadsförings-% ur Awin-textfält (sparar, rea, 40 %, etc.) */
function maxPercentFromMarketingFields(row: Record<string, string | undefined>): number {
...
function pickProductImages(row: Record<string, string | undefined>): {
  image: string;
...
function mapCategory(
  ...
  if (store.includes("ninja") || store.includes("sharkninja")) {
    ...
    if (name.includes("dammsug") || name.includes("vacuum")) return "Hem";
    return "Hem";
  }
...
async function main() {
  ...
  const deals = records
    .map((row: any) => {
      const storeName = normalizeStoreName(String(row.merchant_name || ""));
  ...
  // Sortera först så dedupe behåller bästa rabatten
